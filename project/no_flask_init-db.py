...
def create_db():
    ...
    try:
        ...
        # Триггер 1: При добавлении оценки - обновление прогресса
        triggers_sql_1 = """
        -- Триггер для обновления прогресса при добавлении оценки
        CREATE TRIGGER IF NOT EXISTS assessment_insert_trigger
        ...
        # Триггер 2: При обновлении оценки - обновление прогресса
        triggers_sql_2 = """
        ...
        # Триггер 3: При добавлении нового модуля в курс - уведомление
        triggers_sql_3 = """
        ...
        # Выполняем SQL-запросы для каждого триггера отдельно
        try:
            conn.executescript(triggers_sql_1)
            conn.commit()
            print("Триггер 1 создан успешно.")
        except Exception as e:
            print(f"Ошибка при создании триггера 1: {e}")
            
        try:
            conn.executescript(triggers_sql_2)
            conn.commit()
            print("Триггер 2 создан успешно.")
        except Exception as e:
            print(f"Ошибка при создании триггера 2: {e}")
            
        try:
            conn.executescript(triggers_sql_3)
            conn.commit()
            print("Триггер 3 создан успешно.")
        except Exception as e:
            print(f"Ошибка при создании триггера 3: {e}")
